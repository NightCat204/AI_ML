...
# GPU 离线任务的工作目录不一定是脚本所在目录，需手动切换
_WORK_DIR = os.path.dirname(os.path.abspath(__file__)) if '__file__' in dir() else '/home/jovyan/work'
os.chdir(_WORK_DIR)
...
config = EasyDict({
    ...
    "decay_type": 'cosine', # cosine 退火策略，平滑收敛
    "momentum": 0.9, # 标准动量值
    "weight_decay": 0.0001, # 适度 L2 正则化（原 3.0 过大）
